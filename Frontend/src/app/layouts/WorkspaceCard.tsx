...
export function WorkspaceCard({ icon: Icon, title, description, meta, colorIndex, onClick }: {
  icon: LucideIcon;
  title: string;
  description: string;
  meta: string;
    ...
    <Button variant="outline" onClick={onClick}
      ...
      <span className="flex h-full min-w-0 w-full flex-col items-start gap-3">
        <span className="flex w-full items-center justify-between">
          <span className={`rounded-xl p-3 ${colors[Math.max(0, colorIndex) % colors.length]}`}>
            <Icon className="size-5" />
          </span>
          <ArrowRight className="size-4 text-muted-foreground/50 rtl:rotate-180" />
        </span>
        <span className="w-full break-words text-base font-semibold leading-7">{title}</span>
        <span className="w-full break-words text-sm font-normal leading-6 text-muted-foreground">{description}</span>
        <span className="mt-auto rounded-md bg-muted/60 px-2 py-1 text-xs font-medium text-muted-foreground">{meta}</span>
      </span>
    </Button>
  );
}
