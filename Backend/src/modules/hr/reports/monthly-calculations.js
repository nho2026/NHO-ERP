...
const scheduledMinutes = (employee) => {
    ...
    if (employee.scheduleType === "dynamic" &&
        ...
        return (employee.workSchedule.reduce((sum, day) => sum +
            scheduledMinutes({ ...employee, ...day, workSchedule: undefined }), 0) / employee.workSchedule.length);
    }
...
const employeeLabel = (employee) => `${String(employee.firstName ?? "")} ${String(employee.lastName ?? "")}`.trim();
...
const payrollAmounts = (baseSalary, minutesLost, hoursPerDay = HOURS_PER_DAY, rewardAmount = 0, punishmentAmount = 0) => {
    ...
    return {
        ...
        rewardAmount,
        punishmentAmount,
        netSalary: Math.max(0, baseSalary + rewardAmount - deduction - punishmentAmount),
    };
};
...
const deviceAttendanceRecords = (events, people, employees, month) => {
    ...
    return [...groups.values()].map(({ employeeId, date, events: records }) => {
        const ordered = records.sort((a, b) => new Date(a.occurredAt).getTime() - new Date(b.occurredAt).getTime());
        const checkIn = ordered.find((event) => event.eventType === "check_in")?.occurredAt;
        const checkOut = ordered
            .filter((event) => event.eventType === "check_out")
            .at(-1)?.occurredAt;
        let workedMinutes = checkIn && checkOut
            ? Math.max(0, Math.round((new Date(checkOut).getTime() - new Date(checkIn).getTime()) /
                60000))
            : 0;
        ...
        if (employee?.scheduleType === "dynamic") {
            ...
            for (const event of ordered) {
                ...
                if (event.eventType === "check_in" && opened === null)
                    opened = at;
        ...
        const daySchedule = scheduleForDay(employee ?? { id: employeeId }, new Date(recordYear, recordMonth - 1, recordDay).getDay());
        ...
        if (expectedCheckOut <= expectedCheckIn)
            expectedCheckOut += 86400000;
        const flexible = employee?.scheduleType === "dynamic";
        const lateMinutes = !daySchedule || flexible
            ? 0
            : checkIn
                ? Math.max(0, Math.round((new Date(checkIn).getTime() - expectedCheckIn) / 60000) - (settingsSnapshot()?.hr.graceMinutes ?? 0))
                : expectedMinutes;
        const earlyLeaveMinutes = flexible
            ...
            : !daySchedule
                ? 0
                : checkOut
                    ? Math.max(0, Math.round((expectedCheckOut - new Date(checkOut).getTime()) / 60000))
                    : expectedMinutes;
        return {
            ...
            missingMinutes: flexible && checkIn && checkOut
...
return { activeSalaryFor, deviceAttendanceRecords, lostMinutes, scheduledMinutes, payrollAmounts, employeeLabel };
}
